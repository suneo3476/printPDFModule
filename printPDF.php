...
function extractTitle($str){
    if(preg_match('/^'.toUTF8('ÉJÉeÉSÉä').':.+/u', $str, $matches)==1){
        return $str;
    }else{
...
function extractImagelink($str){
    ...
    preg_match_all('/\[\['.toUTF8('ÉtÉ@ÉCÉã').':(.+?)\|/u', $str, $matches_ja);
...
//            echo $let.':'.$str.'<br>';
//            echo '<br><br>';
            return mb_substr($str, 0, $i, 'UTF-8').toUTF8('Åc');
        }
    }
//    echo '<br><br>';
//    echo $let.':'.$str.'<br>';
...
function confirmPartialBody($value){
    if(preg_match('/^'.toUTF8('ÉJÉeÉSÉä').':.+/u', $value->{'title'})==1){
        return true;
    }else {
        foreach ($value->{'category'} as $key => $cat) {
            if ($cat == toUTF8("èëÇ´Ç©ÇØ") || $cat == toUTF8("î‡")) {
                return true;
                break;
            }
        }
